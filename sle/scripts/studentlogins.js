...
function getQuestions(ClassID){
	...
	xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			...
			document.getElementById('keyList').innerHTML= resp;
		}
	}
	xmlhttp.open("GET","scripts/viewassignedstudentlogins.php?class=" + ClassID ,true);
	xmlhttp.send();
}
function reSetStudent(){
	var x = document.getElementById('SUser').value;
	var xmlhttp;
	if (window.XMLHttpRequest){// code for IE7+, Firefox, Chrome, Opera, Safari
		xmlhttp=new XMLHttpRequest();
	}else{// code for IE6, IE5
	...
	xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			...
			//update the innerHTMl of the questions list
			if(resp=="1"){
				window.location.replace("studentlogins.php?just=Student's responses reset.");
			}else{
				window.location.replace("studentlogins.php?just=Error student's responses not reset.");
			}
		}
	}
	xmlhttp.open("GET","scripts/resetchild.php?Pupil=" + x ,true);
	xmlhttp.send();
}
